...
# `nn.Module.__call__` is typed to return `Any` (its dispatch through
# `_call_impl` is not generic over the wrapped module's own return type), so
# every layer call below needs an explicit cast back to `Tensor`.
...
class _UNetDownBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return cast("torch.Tensor", self.relu(self.bn2(self.conv2(x))))


class _UNetUpBlock(nn.Module):
    def __init__(
        self, prev_channel: int, input_channel: int, output_channel: int
    ) -> None:
        super().__init__()
...
class UNet(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.down_block1 = _UNetDownBlock(3, 16, down_size=False)
        self.down_block2 = _UNetDownBlock(16, 32, down_size=True)
        self.down_block3 = _UNetDownBlock(32, 64, down_size=True)
        self.down_block4 = _UNetDownBlock(64, 128, down_size=True)
        self.down_block5 = _UNetDownBlock(128, 256, down_size=True)
        self.down_block6 = _UNetDownBlock(256, 512, down_size=True)
        self.down_block7 = _UNetDownBlock(512, 1024, down_size=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return cast("torch.Tensor", self.last_conv2(x))
