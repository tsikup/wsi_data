...
def plot_coordinates(
    image, coordinates, border_rgb, thickness=1, fill_rgb=None, alpha=1
):
    overlay = image.copy()
    overlay = cv2.polylines(
        overlay, [np.int32(coordinates)], True, color=border_rgb, thickness=thickness
    )
...
def annotations_to_mask(image, annotations, offset, color_map, ratio, fill=False):
    for annotation in annotations:
        ...
        if fill:
            image = plot_coordinates(
                image, coordinates, border_rgb=rgb_color, fill_rgb=rgb_color
            )
        else:
...
def plot_annotations(
    ...
    if use_base_coordinates:
        ...
        annotations = [
            annotation.translate((min_x, min_y)) for annotation in annotations
        ]
